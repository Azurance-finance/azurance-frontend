...
export const useInsurances = (limit = 100, offset = 0, filter = 'Ongoing') => {
    ...
    const fetchInsurances = useCallback(async () => {
        const result = filter === 'Ongoing' ? await queryActiveInsurance(limit, offset) : await queryInactiveInsurance(limit, offset);
        if (result) setInsurances(result.insurancePools.map(item => ({
            ...item,
            name: item.buyerToken.name.replace('-BUY', ''),
            symbol: item.buyerToken.symbol.replace('-BUY', ''),
            maturityTime: item.maturityBlock * 5,
            staleTime: item.staleBlock * 5
        })));
    }, [limit, offset, filter]);
    ...
    const computedInsurances = useMemo(() => {
        return insurances.map((item) => ({
            ...item,
            maturityTime: item.maturityBlock * secondsPerBlock,
            staleTime: item.staleBlock * secondsPerBlock
        }));
